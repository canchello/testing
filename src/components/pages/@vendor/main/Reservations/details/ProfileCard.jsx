...
import { ROUTES, USER_ROLES, USER_STATUS } from "@/libs/constants";
...
import { faClose, faEllipsis, faEnvelope, faExclamationCircle, faPhone, faTrash, faUser } from "@fortawesome/free-solid-svg-icons";
...
const ProfileCard = ({ user, passport }) => {
  ...
  const createChat = async () => {
    try {
      setChatLoading(true)
      const { data } = await Axios({ ...createChatURL, data: { id: user._id } })
      toast.success(data.message)
      router.push(ROUTES.VENDOR.MESSAGES + `?chat=${data.data?._id}`)
    } catch (error) {
      console.error(error)
    } finally {
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-3">
          {[
            ...
            { title: "Passport No.", value: passport?.passportNumber || "-" },
          ].map((item, index) => (
